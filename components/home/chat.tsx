...
const Chat = () => {
  const { chats, addChat, setCurrentChat, currentChatId, updateName, addMessage } = useChatContext();
  ...
  if (!currentChat) {
    return (
      <div className='min-h-screen flex flex-col items-center justify-center px-10 md:px-20 lg:px-40'>
        ...
        <Textarea ref={inputRef} className='min-h-28 2xl:min-h-40 mt-8 border-header-purple shadow-sm' placeholder='Enter prompt ...'/>
        <Button onClick={handleNewDefaultChat} className='w-50 mt-2 2xl:text-[24px] bg-header-purple text-white' variant='default' size='default'>Go</Button>
      </div>
    )
  }
  ...
  return (
    <div className='min-h-screen flex flex-col items-center justify-center px-10 md:px-20 lg:px-40'>
      ...
      <Textarea ref={inputRef} className='min-h-28 2xl:min-h-40 mt-8 border-header-purple shadow-sm' placeholder='Enter prompt ...'/>
      <Button onClick={handleSend} className='w-50 mt-2 2xl:text-[24px] bg-header-purple text-white' variant='default' size='default'>Go</Button>
    </div>
  );
};
